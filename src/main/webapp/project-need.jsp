...
<title>需求分析管理</title>
                ...
                success:function (msg) {
                    $(msg).each(function (index, item) {
                        var tr = "<tr name='addtr' align='center' bgcolor='#FFFFFF' onMouseMove='javascript:this.bgColor='#FCFDEE';' onMouseOut='javascript:this.bgColor='#FFFFFF';'height='22' >" +
                            ...
                            "<td>"+getMyDate(item.addtime)+"</td>" +
                            "<td>"+getMyDate(item.updatetime)+"</td>" +
                            "<td><a href='project-need-edit.jsp?id="+item.id+"'>编辑</a> | <a href='project-need-look.jsp?id="+item.id+"'>查看详情</a></td>" +
                            "</tr>"
                        $("#mark").before(tr);
                    })
                }
    ...
    function batchdel() {
        var ids =[];
        var checkbox = $("[name=id]:checked");
        $(checkbox).each(function () {
            var id = $(this).val();
            ids.push(id);
        })
        $.ajax({
            url:"${pageContext.request.contextPath}/pneed/batchdel",
            type:"post",
            data:{"ids":ids},
            success:function (msg) {
                    alert(msg.message);
    ...
    function search() {
            ...
            var orderby = $("#orderby").val();
             $("[name=addtr]").remove()
        $.ajax({
            url:"${pageContext.request.contextPath}/pneed/search",
            type:"post",
    ...
    function excela() {
        $.ajax({
            type:"GET",
            url:"${pageContext.request.contextPath}/pneed/excel",
            success:function (msg) {
                //成功的话弹出确认框
...
<td height="28" colspan="12">
	...
	<a href="javascript:excela()" class="coolbg">&nbsp;导出Excel&nbsp;</a>
</td>
</tr>
