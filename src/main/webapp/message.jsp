...
<script type="text/javascript">
    ...
    function pagesadd(msg){
        $(msg.map.page.list).each(function (index, item) {
            ...
            var tr  = 	"<tr align='center' name='addtr' bgcolor='#FFFFFF' onMouseMove='javascript:this.bgColor='#FCFDEE';' onMouseOut='javascript:this.bgColor='#FFFFFF';' height='22' >" +
                "<td><input name='id' type='checkbox' id='"+item.id+"' value='"+item.id+"' class='np' /></td>" +
                "<td>"+(index+1)+"</td>" +
                "<td>"+item.title+"</td>" +
                "<td align='center'><span>"+item.content+"</span></td>" +
                "<td>"+item.ename+"</td>" +
                "<td>"+getMyDate(item.sendtime)+"</td>" +
                "<td><a >删除</a></td>" +
                "</tr>";
            $("#mark").before(tr);
        })
    ...
    function search() {
        ...
        $.ajax({
            url:"${pageContext.request.contextPath}/info/seachBypage",
            data:{"search_cid":search_cid,"search_like_keyword":search_like_keyword},
            success:function (msg) {
                pagesadd(msg)
            }
        })
    ...
    function del() {
        ...
        $.ajax({
            url:"${pageContext.request.contextPath}/info/del",
            type:"post",
    ...
    function excels() {
        $.ajax({
            type:"GET",
            url:"${pageContext.request.contextPath}/info/excels",
    ...
    <td background='skin/images/wbg.gif' align='center'>
      <table border='0' cellpadding='0' cellspacing='0'>
        <tr>
          ...
          <td width='160'>
          <select name='search_cid' id="search_cid" style='width:150px'>
        ...
        <td width='160'>
          	<input type='text' id="search_like_keyword" name='search_like_keyword' value='' style='width:120px' />
        </td>
        <td width='110'>
...
<td height="28" colspan="12">
	&nbsp;
	<a href="javascript:all()" class="coolbg">全选</a>
